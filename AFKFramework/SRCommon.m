...
#define	FailWithNaiveString SRStringForKeyCode(keyCode)
	...
	NSString *resultString = [NSString stringWithString:(NSString *) objc_unretainedObject(mutableTemp)];
...
//	NSLog(@"constructing image");
    ...
    if ([name isEqualToString:@"SRSnapback"]) {
        sizeValue = [self _sizeSRSnapback];
    }
    
    if ([name isEqualToString:@"SRRemoveShortcut"]) {
        sizeValue = [self _sizeSRRemoveShortcut];
    }
    
    if ([name isEqualToString:@"SRRemoveShortcutPressed"]) {
        sizeValue = [self _sizeSRRemoveShortcutPressed];
    }
    
    if ([name isEqualToString:@"SRRemoveShortcutRollover"]) {
        sizeValue = [self _sizeSRRemoveShortcutRollover];
    }
