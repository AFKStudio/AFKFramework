...
- (BOOL) isKeyCode:(NSInteger)keyCode andFlagsTaken:(NSUInteger)flags error:(NSError **)error;
{
	...
	NSArray *globalHotKeys = [NSArray arrayWithArray:(NSArray *) objc_unretainedObject(tempArray)];
	...
	while (( globalHotKeyInfoDictionary = [globalHotKeysEnumerator nextObject] ))
	{
		...
		if ( (CFBooleanRef)objc_unretainedPointer([globalHotKeyInfoDictionary objectForKey:(NSString *) objc_unretainedObject(kHISymbolicHotKeyEnabled)]) != kCFBooleanTrue )
            continue;
        ...
        globalHotKeyCharCode = [(NSNumber *)[globalHotKeyInfoDictionary objectForKey:(NSString *) objc_unretainedObject(kHISymbolicHotKeyCode)] shortValue];
        
        CFNumberGetValue((CFNumberRef) objc_unretainedPointer([globalHotKeyInfoDictionary objectForKey: (NSString *) objc_unretainedObject(kHISymbolicHotKeyModifiers)]),kCFNumberSInt32Type,&globalHotKeyFlags);
